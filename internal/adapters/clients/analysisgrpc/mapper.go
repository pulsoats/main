...
func mapRunMetaResponse(resp *analysispb.RunMeta) (analysis.Run, error) {
	...
	if !ok {
		return analysis.Run{}, fmt.Errorf("map run meta response: %w: resp is nil", derrors.ErrInvalidArgument)
	}

	interval, ok := market.ParseInterval(resp.Interval)
	if !ok {
		return analysis.Run{}, fmt.Errorf("map run meta response: %w: interval %v", derrors.ErrNotFound, resp.Interval)
	}
