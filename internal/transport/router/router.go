...
func NewRouter(cfg Config) (*gin.Engine, error) {
	...
	analysisGroup.GET("/catalog/filters", cfg.AnalysisHandler.AvailableFilters)
	...
	acc.GET("/catalog/filters", cfg.LiveHandler.AvailableFilters)
	...
	acc.GET("/worker/metrics", cfg.LiveHandler.WorkerMetrics)
