...
func (a *Application) checkExpiringAccounts(ctx context.Context) {
	...
	accounts, err := a.accountRepo.Accounts(ctx)
	if err != nil {
		a.logger.Error("failed to fetch accounts", "op", op, "error", err)
	}
	...
	for _, acc := range accounts {
		...
		if acc.ExpiresAt.After(now) && acc.ExpiresAt.Before(now.AddDate(0, 0, 7)) {
			...
			err := a.emailSender.Send(sendCtx, templates.AccountExpiryReminder(acc.Email, a.appName, acc.Name, acc.Exchange, int(time.Until(acc.ExpiresAt).Hours()/24)))
			if err != nil {
				a.logger.Error("failed to send expiry reminder", "op", op, "error", err, "account_id", acc.ID)
			}
			cancel()
		}
		...
		if acc.ExpiresAt.Before(now) {
			sendCtx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
			err := a.emailSender.Send(sendCtx, templates.AccountExpired(acc.Email, a.appName, acc.Name, acc.Exchange))
			if err != nil {
				a.logger.Error("failed to send expiry notification", "op", op, "error", err, "account_id", acc.ID)
			}
			cancel()
		}
	}
